...
habit_coffee: Only 1 Coffee
habit_no_coffee: No Coffee At All
habit_dotnet_guide: Complete Guide to building an App with .NET Core and React
...
habit_song_analysis: Work on the Analysis of a Chord Progression of a Song
habit_to_dos: Get ToDos Done
habit_article: Work through an Article from my Articles and Projects Notes
---
...
| <div style="height: 20px; width: 20px; background-color: #8a30f2; border-radius: 5px; "></div>     | `$= dv.page("Habits").habit_to_dos`          |
| <div style="height: 20px; width: 20px; background-color: #d7a852; border-radius: 5px; "></div><br> | `$= dv.page("Habits").habit_article`         |
...
for(let page of dv.pages('"Daily Notes"').where(p=>p.focus)){
	...
	if (page.focus=="habit_study_plan"){color="green"}
	if (page.focus=="habit_to_dos"){color="purple"}
	if (page.focus=="habit_article"){color="yellow"}
